...
| Tool layer | 66 tools wrapping HEASoft, XSPEC, CIAO, SAS, Fermitools | 47 tools wrapping sunpy, pyspedas, plasmapy, cdasws, sscws, hapiclient, cdflib + HDRL/NOAA REST services |
| Environment | conda env, data trees, outputs, logs | `uv`-managed env; `workspace/{data,outputs,logs}` |
| Knowledge | 317 skill documents | 42 skill documents (missions / methods / datasources / tools), growing |

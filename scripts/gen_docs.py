...
README.md is only partly generated: the script rewrites the block between
the DOCS_INDEX markers and leaves the rest of the file alone.
"""
...
def readme_md() -> str:
    """README.md with only the docs-index block regenerated."""
    text = (ROOT / "README.md").read_text()
    start, end = text.find(DOCS_INDEX_START), text.find(DOCS_INDEX_END)
    if start == -1 or end == -1:
        raise SystemExit(
            f"README.md is missing the {DOCS_INDEX_START} / {DOCS_INDEX_END} "
            "markers; add them where the docs index belongs.")
    return text[:start] + readme_docs_index() + text[end + len(DOCS_INDEX_END):]
